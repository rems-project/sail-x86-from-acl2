...
static int read_req(struct rsp_conn *conn, struct rsp_buf *req) {
  ...
  do_read:
    ...
    if (nbytes == 0) {
      dprintf(conn->log_fd, "[dbg] Connection closed\n");
      // As with D below, maybe wait again?
      conn_exit(conn, 0);
    }
...
static void handle_query(struct rsp_conn *conn, struct rsp_buf *req, struct rsp_buf *resp) {
  if (match_req_cmd(req, "qSupported:")) {
    ...
    append_rsp_buf_msg(conn, resp, ";PacketSize=1024;qXfer:features:read+");
  ...
  if (match_req_cmd(req, "qXfer:features:read:target.xml:")) {
    ...
    if (sscanf(req->cmd_buf, "qXfer:features:read:target.xml:%d,%d", &offset, &length) != EOF) {
      ...
      if (offset > xml_length) {
        ...
        if (length >= xml_length - offset) {
          append_rsp_buf_msg(conn, resp, "l");
          append_rsp_buf_bytes(conn, resp, gdb_xml + offset, xml_length - offset);
          return;
        } else {
          append_rsp_buf_msg(conn, resp, "m");
          append_rsp_buf_bytes(conn, resp, gdb_xml + offset, length);
          return;
        }
      }
    }
...
static void handle_cont(struct rsp_conn *conn, struct rsp_buf *req, struct rsp_buf *resp) {
  ...
  if (req->cmd_buf[1] != '#') {
    ...
    int ofs = 1;
    if (extract_hex_integer_be(conn, req, &ofs, '#', &addr) < 0) {
      dprintf(conn->log_fd, "badly formatted continue command\n");
      make_error_resp(conn, resp, 1);
    }
    arch->set_pc(conn, arch, addr);
  }
...
static void handle_sw_breakpoint(struct rsp_conn *conn, struct rsp_buf *req, struct rsp_buf *resp) {
  if (match_req_cmd(req, "Z0,") || match_req_cmd(req, "z0,")) { // software breakpoint
    ...
    ofs++;
    // If we had conditional triggers I'd need both terminators...
    if (extract_hex_integer_be(conn, req, &ofs, '#', &kind) < 0) {
      dprintf(conn->log_fd, "internal error: bad breakpoint kind found\n");
      make_error_resp(conn, resp, 1);
      return;
    }
